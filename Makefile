...
build-dev-mode: ## Build and run application in development mode
	...
	docker exec montana-php php artisan key:generate
...
full-dev-rebuild: ## Full re-install application
	...
	docker exec montana-php php artisan migrate:fresh --seed
	docker exec montana-php php artisan storage:unlink
	docker exec montana-php php artisan storage:link

migrate-seed: ## Run migration and seeders
	docker exec montana-php php artisan migrate:fresh --seed

backend-cache-clear: ## Clean backend application cache
	docker exec montana-php php artisan cache:clear
...
# при подписке или повторяемой трате, юзер может выбрать была ли оплата в текущий период уже
# если нет, то дату ставим как null, если да то ставим текущую
